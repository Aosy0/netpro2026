...
public class OmikujiHttpServer {
  ...
  static class OmikujiHandler implements HttpHandler {
    ...
    private static final List<String> FORTUNES = List.of("大吉", "中吉", "小吉", "吉", "末吉", "凶");

    // おみくじごとのメッセージ（キーと完全一致させること）
    private static final Map<String, String> MESSAGES = Map.of("大吉", "最高の運気。何をやっても吉。", "中吉", "穏やかな幸運。焦らず進め。", "小吉",
        "徐々に良くなる兆しあり。", "吉", "平凡な日常。小さな喜びを見つけて。", "末吉", "行動次第で吉も凶も。慎重に。", "凶", "用心せよ。無理は禁物。");
    ...
    public void handle(HttpExchange exchange) throws IOException {
      ...
      if (!"GET".equalsIgnoreCase(exchange.getRequestMethod())) {
        ...
        return;
      } else if ("ATAREEEEEE".equals(exchange.getRequestHeaders().getFirst("X-Custom-Header"))) {
        // 文字列の方から equals() を呼び出さないとエラーになる
        fortune = FORTUNES.get(0);
      } else {
        // --- 2. ランダムにおみくじを選択 (『凶』は 1% の確率で返す) ---
        fortune = selectFortuneWithProbability();
        // TODO: selectFortuneWithProbability() を実装し、以下のいずれかを返すこと
        // 「大吉」「中吉」「小吉」「吉」「末吉」「凶」
        // ・「凶」は全体の 1% の確率
      ...
      // --- 4. JSON を組み立て ---
      /*
       * 以下のキーを持つ JSONObject を作成すること { "status" : 200, "fortune": fortune, "message":
       * message }
       */
      JSONObject json = new JSONObject();
      // TODO: json.put("status", ...);
      // TODO: json.put("fortune", ...);
      // TODO: json.put("message", ...);
      json.put("status", 200);
    ...
    /**
     * 〈補助メソッド〉 1% の確率で「凶」を返し、残り 99% を 5 つに等分して返すロジック。 このメソッドを完成させること。
     */
