...
public class RepeatChecker {
  ...
  public static boolean isTripleRepeat(String text) {
    ...
    if (text.length() % 3 != 0) {
      ...
      if (part.equals(text.substring(text.length() / 3, text.length() * 2 / 3))
          && part.equals(text.substring(text.length() * 2 / 3))) {
        return true;
      } else {
        return false;
      }
  ...
  public static boolean isSemiTripleRepeat(String text) {
    ...
    if (text.length() % 3 != 0) {
      ...
      String part = text.substring(0, text.length() / 3).toLowerCase();
      if (part.equals(text.substring(text.length() / 3, text.length() * 2 / 3).toLowerCase())
          && part.equals(text.substring(text.length() * 2 / 3).toLowerCase())) {
        return true;
      } else {
        return false;
      }
